...
import { PROFICIENCY_RANK } from '../constants/common.js';
import { getLastMonday } from '../utils/date.js';
...
export class AiService {
  constructor(
    ...
    private readonly configRepo: typeof systemConfigRepository,
    private readonly projectRepo: typeof projectRepository
  ) {}

  private getModelUsed(list: string[]): string {
    return Array.from(new Set(list)).join(', ') || 'None';
  }

  async skillMatch(requirement: string, projectId?: string, requestingManagerId?: string) {
    const modelsUsedList: string[] = [];
    const config = await this.configRepo.getConfig();
    const maxWeeklyHours = config?.maxWeeklyHours || 40;

    const dbSkillsList = await this.skillRepo.findAll();
    ...
    try {
      ...
      console.error('[AiService] Extraction failed, falling back:', err);
    }
    ...
    const resources = await this.resourceRepo.findAll({ isActive: true });
    const activeAllocations = await this.allocationRepo.findAll({ status: 'ACTIVE' });
    ...
    if (extracted.requiredSkills.length > 0 && dbSkills.length === 0) {
      return {
        ...
        modelUsed: this.getModelUsed(modelsUsedList)
      };
    }
    ...
    try {
      ...
      const augmentedResults = (parsed.results || []).map((resObj) => {
        ...
        const isUnderManager = foundResource && foundResource.managerId && foundResource.managerId.toString() === requestingManagerId;
        return {
          ...resObj,
      ...
      return { success: true, summary: parsed.summary, results: augmentedResults, modelUsed: this.getModelUsed(modelsUsedList) };
    } catch {
  ...
  async riskSummary(projectId: string) {
    const modelsUsedList: string[] = [];
    const project = await this.projectRepo.findById(projectId);
    if (!project) {
    ...
    try {
      ...
      modelsUsedList.push(llmService.lastModelUsed);
      return { success: true, summary: summary.trim(), modelUsed: this.getModelUsed(modelsUsedList) };
    } catch {
      return { success: true, summary: buildFallbackSummary(), modelUsed: this.getModelUsed(modelsUsedList) };
    }
  }

  async teamMatch(requirement: string, projectId?: string) {
    const modelsUsedList: string[] = [];
    const config = await this.configRepo.getConfig();
    const maxWeeklyHours = config?.maxWeeklyHours || 40;
    ...
    try {
      ...
      console.error('[AiService] Team Extraction failed, falling back:', err);
    }
    ...
    const candidatesPerRole: Record<string, ICandidate[]> = {};
    ...
    try {
      ...
      const enrichedAssignments = (parsed.assignments || []).map((assignment: IAssignment) => {
        if (assignment.status === 'FILLED' && assignment.assignedResource) {
          const resId = assignment.assignedResource.resourceId;
          const foundResource = resources.find(r => r._id.toString() === resId);
          let candDetails: ICandidate | null = null;
          for (const roleName of Object.keys(candidatesPerRole)) {
            const list = candidatesPerRole[roleName] || [];
      ...
      return { success: true, summary: parsed.summary, assignments: enrichedAssignments, modelUsed: this.getModelUsed(modelsUsedList) };
    } catch {
